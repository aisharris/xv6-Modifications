...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    //intializing to avoid warning :/
    struct proc* minproc = &proc[0];
    uint minctime = minproc->ctime;
    uint num = (3*minproc->runtime - minproc->stime - minproc->wtime)*50;
    uint den = minproc->runtime + minproc->stime + minproc->wtime + 1;
    uint rbi = num/den;
    uint mindp = 200; //doesnt matter im reinitializing later

    //initialize everything to first runnable process 
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        ...
        release(&p->lock);
        break; //after first runnble process is found, break
      }
      release(&p->lock);
    }

    uint dp = 100; //min(100, sp + rbi)

    //loop through all to find min dp
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        //calculate dp
        //rbi = max of 0 or int((3*rtime - stime - wtime)/(rtime + stime + wtime)*50)
        //dp = min of 100 or sp + rbi
    ...
    //if none of them were runnable you have to continue, so make sure there was a runnable process
    if (minproc->state == RUNNABLE)
    {
      ...
      // printf("scheduling %d %d %d %d %d\n", minproc->pid, minproc->runtime, minproc->wtime, minproc->stime, dp);
