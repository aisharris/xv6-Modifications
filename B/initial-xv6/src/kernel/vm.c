...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    pa = PTE2PA(*pte);
    //instead of copying and making new pages, make childs pages map to parents.
    //increase ref for the same physical page

    //function to increase ref count
    increase_ref((void*)pa);

    // if(*pte & PTE_W) //if page is writable
    // {
    //   //set permissions:
